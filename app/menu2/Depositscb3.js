...
function updateDisplay() {
    ...
    const formattedDate = formatDate(datetime.substring(0, 10)); // สำหรับรายการแรก
    const formattedDate1 = formatDate(datetime1.substring(0, 10)); // สำหรับรายการที่สอง
    const formattedDateWithDay = formatDateWithDay(datetime.substring(0, 10)); // สำหรับหัวข้อ
    const formattedTime = datetime.substring(11, 16); // เวลาในรายการแรก
    const formattedTime1 = datetime1.substring(11, 16); // เวลาในรายการที่สอง
    ...
    backgroundImage.onload = function() {
       ...
       3
        ...
        drawText(ctx, `เงินเข้า: มีเงินโอน ${money01} บาท จาก ${name1} บัญชี ${receiveraccount} ธนาคาร ${bank1} เข้าบัญชีออมทรัพย์ ${senderaccount1} วันที่ ${formattedDate} @${formattedTime} ผ่าน ENET ยอดเงินที่ใช้ได้ ${money10} บาท
            `, 107.8, 749,20.50, 'SFThonburiRegular', '#000000', 'left', 31.5, 3, 0, 0, 425, -0.25);
            ...
            drawText(ctx, `เงินเข้า: มีเงินโอน ${money02} บาท จาก ${name2} บัญชี ${receiveraccount1} ธนาคาร ${bank2} เข้าบัญชีออมทรัพย์ ${senderaccount1} วันที่ ${formattedDate1} @${formattedTime1} ผ่าน ENET ยอดเงินที่ใช้ได้ ${money11} บาท
                `, 107.8, 950,20.50, 'SFThonburiRegular', '#000000', 'left', 31.5, 3, 0, 0, 425, -0.25);
...
// วาดการเติมแบตเตอรี่
const fillWidth = (batteryLevel / 100) * 32.5; // คำนวณความกว้างของการเติมแบตเตอรี่ตามระดับแบตเตอรี่
const x = 511.5;
const y = 32.4;
const height = 13.8;
